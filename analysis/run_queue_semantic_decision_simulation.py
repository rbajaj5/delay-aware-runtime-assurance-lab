"""Finite-capacity message queues with downstream stale-decision cost.

This is an exploratory systems diagnostic, not a safety certificate. Each
agent tracks the current binary state of its two ring neighbors. Useful
updates carry the sender's state; background messages consume the same
service capacity. The receiver acts using its latest applied neighbor states.
"""
...
def choose_due(
    queue: list[Message], policy: str, now: int
    ...
    if policy == "fifo_blind":
        ...
        chosen = (useful + background)[:CAPACITY]
    chosen_ids = {message.sequence for message in chosen}
    remaining = future + [message for message in due if message.sequence not in chosen_ids]
...
def run_game(delay: int, background_rate: float, policy: str, seed: int) -> dict[str, float | int | str]:
    ...
    for now in range(ROUNDS):
        # A small endogenous change makes stale state consequential rather than
        # merely a delivery-label distinction.
        ...
        for receiver in range(AGENTS):
            ...
            for message in chosen:
                if message.useful:
                    useful_processed += 1
                    useful_wait += now - message.created
                    useful_wait_count += 1
        ...
        for sender in range(AGENTS):
            for receiver in neighbors(sender):
                ...
                queues[receiver].append(
                    Message(receiver, sender, now, now + delay, state[sender], True, sequence)
                )
            for _ in range(poisson(rng, background_rate)):
                sequence += 1
                queues[sender].append(
                    Message(sender, None, now, now, None, False, sequence)
                )
    ...
    return {
        ...
        "mean_useful_wait": useful_wait / useful_wait_count if useful_wait_count else 0.0,
...
def aggregate(rows: list[dict[str, float | int | str]]) -> list[dict[str, float | int | str]]:
    groups: dict[tuple[int, float, str], list[dict[str, float | int | str]]] = defaultdict(list)
    for row in rows:
...
def render(summary: list[dict[str, object]], path: Path) -> None:
    ...
    panels = (
        ...
        ("mean_background_processed", "Background processed", "messages / agent / game"),
        ("mean_backlog", "Total queue backlog", "messages"),
    )
    colors = {"fifo_blind": "#d55e00", "priority_deduplicating": "#0072b2"}
    labels = {"fifo_blind": "blind FIFO", "priority_deduplicating": "priority + dedup"}
    for axis, (field, title, ylabel) in zip(axes.flat, panels):
        for policy in POLICIES:
            values = [float(next(row[field] for row in subset if float(row["background_rate"]) == rate and row["policy"] == policy)) for rate in rates]
        ...
        axis.legend()
    fig.suptitle(f"Queue-aware decisions under background traffic (delay = {delay})", fontsize=16, fontweight="bold")
    fig.text(0.01, 0.015, "12-agent ring; capacity 2/agent/round; 20 games/cell; exploratory only.", fontsize=9)
...
def main() -> None:
    ...
    for delay in DELAYS:
        for rate in BACKGROUND_RATES:
            for policy in POLICIES:
                for game in range(GAMES_PER_CELL):
                    rows.append(dict(run_game(delay, rate, policy, seed)))
                    seed += 1
    summary = aggregate(rows)
    raw_path = OUT / "queue_semantic_decision_games.csv"
    summary_path = OUT / "queue_semantic_decision_summary.csv"
    figure_path = OUT / "queue_semantic_decision.png"
    report_path = OUT / "queue_semantic_decision_report.md"
    manifest_path = OUT / "QUEUE_SEMANTIC_DECISION_MANIFEST.sha256"
